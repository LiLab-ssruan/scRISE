...
def calculateKNNgraphDistanceMatrixStatsSingleThread(featureMatrix, distanceType='euclidean', k=10, param=None):
    ...
    for i in np.arange(featureMatrix.shape[0]):
        ...
        tmp=featureMatrix[i,:].reshape(1,-1) # 每一行转换成一列
        distMat = distance.cdist(tmp, featureMatrix,distanceType)
        #distMat = distance.cdist(normalize(tmp-tmp.mean(axis=1)[:, None]),normalize(featureMatrix-featureMatrix.mean(axis=1)[:, None]), distanceType) # 计算距离
        res = distMat.argsort()[:k+1] # 返回从小到大的K+1个索引值
        tmpdist = distMat[0,res[0][1:k+1]]
        boundary = np.mean(tmpdist)+np.std(tmpdist) # 均值＋标准差
        for j in np.arange(1,k+1):
...
def pearson_correlation(X):
    ...
    # Set the diagonal to zero, since correlation with oneself is always 1
    np.fill_diagonal(corr_matrix, 0)

    return corr_matrix


def cknn_graph(X, k, delta=1, distanceType='euclidean'):
    ...
    """
    assert k < X.shape[0] #断言,如果表达式为false，则抛出java.lang.AssertionError异常

    D = distance.cdist(X, X, distanceType)
    N = D.shape[0] #行数
    np.fill_diagonal(D, 0) #将numpy数组的对角线填充为参数中传递的值，对角线为0
    D_k = np.sort(D) #每一行按从小到大排列
    adj = np.zeros([N, N])  #创建矩阵
    adj[np.square(D) < delta * delta * np.dot(D_k[:, k].reshape(-1, 1), D_k[:, k].reshape(1, -1))] = 1
    np.fill_diagonal(adj, 0)
    return adj

def pearson_cknn_graph(X, k, delta=1):
    corr = np.corrcoef(X)
    D = 1 - corr
    N = D.shape[0]
...
def knn_graph(featureMatrix, k):
    ...
    #     x_std = np.std(x)
    #     y_std = np.std(y)
    #     return np.sum((x - x_mean) * (y - y_mean)) / ((len(x) - 1) * x_std * y_std)

    # edgeList = []

    # for i in range(featureMatrix.shape[0]):
    #     tmp = featureMatrix[i, :].reshape(1, -1)
    #     corrMat = np.apply_along_axis(lambda x: pearson_correlation(tmp.flatten(), x.flatten()), axis=1, arr=featureMatrix)
    #     res = np.argsort(corrMat)[::-1][:k+1]
    #     tmpcorr = corrMat[res[1:]]
    #     boundary = np.mean(tmpcorr) + np.std(tmpcorr)
    #     for j in range(k):
    #         if corrMat[res[j+1]] >= boundary:
    #             weight = 1.0
    #         else:
    #             weight = 0.0
    #         edgeList.append((i, res[j+1], weight))

    corrMat = np.corrcoef(featureMatrix)

    edgeList = []

    for i in range(featureMatrix.shape[0]):
        res = np.argsort(corrMat[i])[::-1][:k+1]
        tmpcorr = corrMat[i][res[1:]]
        boundary = np.mean(tmpcorr) + np.std(tmpcorr)
        for j in range(k):
            if corrMat[i][res[j+1]] >= boundary:
                weight = 1.0
            else:
                weight = 0.0
            edgeList.append((i, res[j+1], weight))
